<?php
  ...
  #$query = "SELECT cod_objeto, codigo, nombre, modelo,fecha_ingresoSistema,proveedor,observacion, Estado.nombre_estado FROM objeto left join Estado on (objeto.estado = Estado.cod_estado)";
  if(isset($_POST['consulta'])){
    $q = $mysqli->real_escape_string($_POST['consulta']);
    $query = "SELECT * FROM objeto  WHERE codigo LIKE '%".$q."%' OR nombre LIKE '%".$q."%'";
  }
  $resultado = $mysqli->query($query) or die("fallo");
